...
class Parser:
    ...
    def get_data(self):
        ...
        for sent in self.sents_with_pos:
            ...
            for entity_mention in self.entity_mentions:
                ...
                if (
                    text_position[0] <= entity_position[0]
                    and entity_position[1] <= text_position[1]
                ):
            ...
            for event_mention in self.event_mentions:
                ...
                if (
                    text_position[0] <= event_position[0]
                    and event_position[1] <= text_position[1]
                ):
                    event_arguments = []
                    for argument in event_mention["arguments"]:
                        try:
                            entity_type = entity_map[argument["entity-id"]][
                                "entity-type"
                            ]
                        except KeyError:
                            print(
                    ...
                    item["golden-event-mentions"].append(
                        {
                            ...
                            "event-id": event_mention["event-id"],
                        }
                    )
            data.append(item)
        return data
    ...
    def find_correct_offset(self, sgm_text, start_index, text):
        ...
        for i in range(0, 70):
            for j in [-1, 1]:
                offset = i * j
                if (
                    sgm_text[start_index + offset : start_index + offset + len(text)]
                    == text
                ):
                    return offset

        print(
            "[Warning] fail to find offset! (start_index: {}, text: {}, path: {})".format(
                start_index, text, self.path
            )
        )
        return offset
        ...
        with open(sgm_path, "r") as f:
            ...
            for sent in sents:
                ...
                sents_with_pos.append(
                    {"text": sent, "position": [pos, pos + len(sent)]}
                )
    ...
    def parse_entity_tag(node):
        ...
        for child in node:
            ...
            entity_mention["entity-type"] = "{}:{}".format(
                node.attrib["TYPE"], node.attrib["SUBTYPE"]
            )
            entity_mention["text"] = charset.text
            entity_mention["position"] = [
                int(charset.attrib["START"]),
                int(charset.attrib["END"]),
            ]
            entity_mention["head"] = {
                "text": head_charset.text,
                "position": [
                    int(head_charset.attrib["START"]),
                    int(head_charset.attrib["END"]),
                ],
            }
    ...
    def parse_event_tag(node):
        ...
        for child in node:
            if child.tag == "event_mention":
                event_mention = dict()
                event_mention["event_type"] = "{}:{}".format(
                    node.attrib["TYPE"], node.attrib["SUBTYPE"]
                )
...
if __name__ == "__main__":
    ...
    parser = Parser(
        "./data/ace_2005_td_v7/data/English/un/timex2norm/alt.corel_20041228.0503"
    )
